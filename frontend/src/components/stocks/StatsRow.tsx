...
import type { Quote, BasicFinancials, CompanyProfile } from '@/types/stock'
...
export function StatsRow({ quote, financials, profile }: StatsRowProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
      ...
      <StatCell label="Beta"      value={financials?.beta != null ? financials.beta.toFixed(2) : '—'} />
    </div>
  )
}
...
export function FinancialGrid({ financials }: FinancialGridProps) {
  const items = [
