...
Route order matters — /history must be declared BEFORE /{ticker}
to prevent FastAPI treating 'history' as a ticker value.
"""
...
# ─── /history must come BEFORE /{ticker} ─────────────────────────────────────
# FastAPI matches routes in declaration order. If /{ticker} is first,
# GET /sentiment/AAPL/history would set ticker="AAPL/history" instead of routing here.
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            ...
            .where(
                ...
                SentimentScore.window_hours == window,
            )
