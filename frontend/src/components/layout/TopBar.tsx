      ...
      {data?.results?.map((result) => (
        result.ticker && (
          ...
          >
            ...
            <span className="font-mono text-sm font-semibold text-text-primary w-16 shrink-0">
              {result.ticker}
            </span>
...
export default function TopBar() {
  ...
  const { addRecentTicker } = useAppStore()
  ...
  return (
    <header className="h-14 border-b border-bg-border bg-bg-surface/80 backdrop-blur-sm flex items-center px-4 gap-4 shrink-0">
      ...
      {/* Right side slots (auth buttons go here in Phase 5) */}
