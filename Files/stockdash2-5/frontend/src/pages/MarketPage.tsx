...
import { useMarketOverview, useMarketSparklines } from '@/hooks/useStockData'
import { useAppStore } from '@/store/useAppStore'
import { formatPrice, formatPct, formatMarketCap, formatVolume, getPriceClass, cn } from '@/lib/utils'
...
function SortIcon({ col, current, dir }: { col: SortKey; current: SortKey; dir: SortDir }) {
  ...
  return dir === 'asc' ? <ArrowUp size={11} className="text-accent-cyan" /> : <ArrowDown size={11} className="text-accent-cyan" />
}

function Sparkline({ candles, isUp }: { candles: { close: number }[]; isUp: boolean }) {
  if (!candles || candles.length < 2) return <div className="w-16 h-8" />
  return (
    <div className="w-16 h-8">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={candles}>
          <Line type="monotone" dataKey="close" stroke={isUp ? '#00ff88' : '#ff3b5c'}
...
export default function MarketPage() {
  ...
  const tickers = useMemo(() => data?.items?.map((r: any) => r.ticker) ?? [], [data])
  const { data: sparklines } = useMarketSparklines(tickers)
  ...
  const rows = useMemo(() => {
    ...
    if (sector !== 'All') items = items.filter((r: any) => r.sector === sector)
    if (search) {
      const q = search.toUpperCase()
      items = items.filter((r: any) => r.ticker?.includes(q) || r.company_name?.toUpperCase().includes(q))
    }
  ...
  const Th = ({ label, col }: { label: string; col: SortKey }) => (
    <th className="text-left py-2.5 px-3 text-[10px] uppercase tracking-wider text-text-muted font-medium cursor-pointer hover:text-text-primary transition-colors select-none whitespace-nowrap"
      onClick={() => handleSort(col)}>
      <span className="flex items-center gap-1.5">{label}<SortIcon col={col} current={sortKey} dir={sortDir} /></span>
    </th>
  )
      ...
      <div className="flex items-center gap-2">
        ...
        {data && (
          <span className="font-mono text-[10px] text-text-muted px-2 py-0.5 rounded-full bg-bg-hover">
            {rows.length} stocks · auto-refresh 60s
          </span>
        )}
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-bg-border bg-bg-surface/50">
                <tr>
                  ...
                  <th className="text-left py-2.5 px-3 text-[10px] uppercase tracking-wider text-text-muted font-medium hidden lg:table-cell">7D Chart</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row: any) => {
                  const inWl   = isInWatchlist(row.ticker)
                  const candles = sparklines?.[row.ticker] ?? []
                    ...
                    <tr key={row.ticker}
                      ...
                      <td className={cn('py-3 px-3 font-mono font-semibold tabular-nums', getPriceClass(row.change_pct))}>
                        <span className="flex items-center gap-1">
                          {isUp ? <TrendingUp size={11} /> : <ArrowDown size={11} />}
