...
class TestWatchlistServiceLayer:
    ...
    async def test_ensure_default_watchlist_creates_when_empty(self, db_session):
        ...
        from app.routers.watchlist import _ensure_default_watchlist

        # New implementation auto-creates guest user internally
        wl = await _ensure_default_watchlist(db_session)
        assert wl.id is not None
        assert isinstance(wl.name, str) and len(wl.name) > 0
    ...
    async def test_ensure_default_watchlist_idempotent(self, db_session):
        ...
        # Calling twice returns the same watchlist (idempotent)
        wl1 = await _ensure_default_watchlist(db_session)
        wl2 = await _ensure_default_watchlist(db_session)
        assert wl1.id == wl2.id

    @pytest.mark.asyncio
    async def test_reorder_persists_to_db(self, db_session):
        """Test that reordering items updates sort_order in DB."""
        from app.models.models import WatchlistItem
        from app.routers.watchlist import _ensure_default_watchlist, _ensure_guest_user

        # Set up a watchlist with items using the auto-create helpers
        await _ensure_guest_user(db_session)
        wl = await _ensure_default_watchlist(db_session)
        wl_id = wl.id

        for i, ticker in enumerate(["AAPL", "MSFT", "NVDA"]):
            db_session.add(WatchlistItem(watchlist_id=wl_id, ticker=ticker, sort_order=i))
        await db_session.flush()
