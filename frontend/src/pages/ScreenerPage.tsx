...
export default function ScreenerPage() {
  ...
  useEffect(() => {
    if (filters.length > 0) {
      setSearchParams({ filters: btoa(JSON.stringify(filters)) }, { replace: true })
    } else {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      const headers = ['Ticker','Company','Price','P/E','Market Cap','Beta','Revenue Growth %','Today %','RSI','Signal']
      const csv = [headers, ...rows.map((r: any) => [
        r.ticker, r.company_name ?? '', r.price, r.pe_ratio, r.market_cap,
        r.beta, r.revenue_growth, r.change_pct, r.rsi, r.analyst_signal,
      ])].map(row => row.join(',')).join('\n')
      const blob = new Blob([csv], { type: 'text/csv' })
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement('a'); a.href = url
      a.download = 'screener-results.csv'; a.click()
