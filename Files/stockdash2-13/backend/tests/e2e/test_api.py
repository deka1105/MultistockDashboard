...
class TestSentimentEndpoints:
    ...
    async def test_sentiment_returns_data(self, client):
        """Sentiment always returns data — mock fallback when StockTwits unreachable."""
    ...
    @pytest.mark.asyncio
    async def test_sentiment_history_always_returns_data(self, client):
        """History now returns mock data instead of empty list when no DB records."""
        r = await client.get("/api/v1/sentiment/UNKNOWNTICKER999/history")
        assert r.status_code == 200
        data = r.json()
        assert "history" in data
        assert isinstance(data["history"], list)
        # Mock fallback populates history so chart always has something to show
